...
export default function HomePage() {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          type="line"
          xKey="period"
          yKeys={[{ key: 'value', name: 'MMboe' }]}
          title="Discovery Volume (Annual)"
        />
        <Chart
          data={data?.categories || [{ category: 'Loading', count: 0 }]}
          type="bar"
          xKey="category"
          yKeys={[{ key: 'count', name: 'Wells' }]}
          title="Well Results by Basin"
        />
      </div>
      <DataTable
        columns={[
  ...
  const domainTab1 = (
    <div className="space-y-6">
      ...
      <Chart
        ...
        type="area"
        xKey="x"
        yKeys={[{ key: 'y', name: 'EMV (RM M)' }]}
        title="Prospect Ranking by Expected Value"
        height={400}
      />
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          title="Well Cost vs Performance"
        />
        <ActionMemo
          persona={{ name: 'Dr. Amir Hamzah', role: 'VP Exploration' }}
          context={{}}
          onGenerate={async () => ({
            subject: 'Action Required',
            body: 'AI-generated recommendation based on current data patterns and predicted trends.',
            urgency: 'HIGH',
            actions: ['Approve Deepwater Block PM-4 spud date', 'Integrate new 3D seismic for Sarawak basin', 'Assess farm-in opportunity for SK-412'],
          })}
        />
      </div>
    </div>
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'Which prospects have the highest expected value?',
          'Show exploration success rate vs industry benchmark',
          'What is the optimal drilling sequence for 2025 program?',
        ]}
        mode="both"
  ...
  const tabs = [
    ...
    { id: 'domain-1', label: 'Subsurface Analytics', icon: '📈', content: domainTab1 },
    { id: 'domain-2', label: 'Drilling Planning', icon: '⚡', content: domainTab2 },
